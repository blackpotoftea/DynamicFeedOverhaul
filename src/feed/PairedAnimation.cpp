...
namespace PairedAnimation {
    ...
    // Fire the vampire-overhaul integration for a completed feed. Shared by the
    // legacy path and the composite path, called once from MarkFeedEnded() after
    // the feed is done. Must run on the main thread.
    void RunFeedIntegration(RE::Actor* callbackTarget, bool isLethal, bool hasOARAnimation) {
        if (!callbackTarget) {
    ...
    void ExecuteFeed(const char* idleEditorID, RE::Actor* target, bool isPairedAnim, bool isLethal, bool hasOARAnimation) {
        ...
        auto onAnimationResult = [](bool success, RE::Actor* /*callbackTarget*/) {
            ...
            // feed completes (see PairedAnimation::RunFeedIntegration).
